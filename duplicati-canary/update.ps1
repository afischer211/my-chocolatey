...
function global:au_GetLatest {
	# Use GitHub API to get the release list and pick the newest canary-tagged release.
	...
	try {
		...
		if (-not $canaryRelease) {
			throw "No canary release found in API response"
		}
	} catch {
		Write-Warning "Failed to access GitHub API, falling back to web scraping"
		# Fallback to web scraping if the API fails, filtered to canary tags only.
		$download_page = Invoke-WebRequest -Uri https://github.com/duplicati/duplicati/releases/ -UseBasicParsing
		$tag_url = $download_page.links | ? href -match 'releases/tag/v.*_canary_' | % href | select -First 1

		if (-not $tag_url) {
			throw "No canary release tag found on the releases page"
		}

		$version = ($tag_url -split '/' | select -Last 1).trim('v')
		$cleanVersion = ($version -replace '_canary.*') + '-canary'
	...
	$version = $canaryRelease.tag_name.TrimStart('v')
	$cleanVersion = ($version -replace '_canary.*') + '-canary'

	# Look for MSI files in the assets
	$msiAsset = $canaryRelease.assets | Where-Object {
